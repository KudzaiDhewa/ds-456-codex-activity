...
We gave it the dataset's URL and it assumed the dataset was downloaded and was on our computer. It also assumed the name of the dataset and that the required packages i.e tidyverse and skimr where installed.

- Did it make any mistakes or questionable choices?

It assumed the dataset had longitude and latitude variables and therefore there was an error with the geom-point graph.

- How clear or explainable was the code it produced?
The code was pretty explainable and it added comments for all the code. However, the execution was off.
...
It edited the code given our instructions pretty well, made it more understanable. Also, it did a good job in choosing the type of charts. They made sense for the task
- What would you have done differently if you wrote the code yourself?

I would've mapped the data for the longitiude and latitude data because it's difficult to understand using longitide and latitude as x and y variables. 
- Looking at the code Codex wrote, what could be improved (style, clarity, correctness, efficiency)?

Facteing the police incidents by month, so it's easier to see the changes by day. Additionally, the function skim kept returning an error despite the package being downloaded. If that could be resolved, it would improve the efficiency.
...
Yes, it does correctly identify what is needed to make the map. However, it assumes the shapefile for Minneapolis exists and uses the geom_sf function on a non-existent shapefile. 
